...
void SilentOtExtReceiver::setBaseOts(span<std::array<block, 2>> baseSendOts) {
...
void SilentOtExtReceiver::genBaseOts(PRNG &prng,
  ...
  mOtExtRecver.genBaseOts(prng, chl);
}
...
void SilentOtExtReceiver::genSilentBaseOts(PRNG &prng,
                                           std::shared_ptr<Channel> chl,
                                           bool useOtExtension) {
  // MC_BEGIN(task<>, this, &prng, &chl, useOtExtension,
  //          choice = sampleBaseChoiceBits(prng), msg =
  //          AlignedUnVector<block>{}, base = DefaultBaseOT{});
...
void SilentOtExtReceiver::configure(u64 numOTs, u64 scaler, u64 numThreads,
  ...
  case MultType::slv11: {
    ...
    u64 gap;
    SilverConfigure(numOTs, 128, mMultType, mRequestedNumOts, mNumPartitions,
...
void SilentOtExtReceiver::checkRT(std::shared_ptr<Channel> chl,
                                  MatrixView<block> rT1) {
  // MC_BEGIN(task<>, this, &chl, rT1,
  //          rT2 =
  //              Matrix<block>(rT1.rows(), rT1.cols(),
  //              AllocType::Uninitialized),
  //          delta = block{}, i = u64{}, R = Matrix<block>{},
  //          exp = Matrix<block>{}, failed = false);
  ...
  {
    auto fut = chl->asyncRecv(rT2);
    auto status = fut.get();
    if (!status.IsOK()) {
      LOG(ERROR) << "Recv rT2 failed.";
      throw std::runtime_error("Recv rT2 failed.");
    }
  }
...
void SilentOtExtReceiver::receive(const BitVector &choices,
  ...
  BitVector randChoice(messages.size());
  ...
  auto status = chl->asyncSend(std::move(randChoice));
  if (!status.IsOK()) {
    LOG(ERROR) << "Send randChoice failed.";
    throw std::runtime_error("Send randChoice failed.");
  }
}

void SilentOtExtReceiver::silentReceive(BitVector &choices,
                                        span<block> messages, PRNG &prng,
                                        std::shared_ptr<Channel> chl,
                                        OTType type) {
  // MC_BEGIN(task<>, this, &choices, messages, &prng, &chl, type,
  //          packing = (type == OTType::Random) ? ChoiceBitPacking::True
  //                                             : ChoiceBitPacking::False);
  ChoiceBitPacking packing = (type == OTType::Random) ? ChoiceBitPacking::True
                                                      : ChoiceBitPacking::False;
...
void SilentOtExtReceiver::silentReceiveInplace(
  ...
  // MC_BEGIN(task<>, this, n, &prng, &chl, type, gapVals =
  // std::vector<block>{},
  //          rT = MatrixView<block>{}, i = u64{}, j = u64{}, main = u64{});
  ...
  //	MC_AWAIT(mGen.expand(chl, prng, rT,
  ...
  // else
  {
    ...
    // MC_AWAIT(mGen.expand(chl, prng, mA.subspan(0, main),
    //                      PprfOutputFormat::Interleaved, true, mNumThreads));
    mGen.expand(chl, prng, mA.subspan(0, main), PprfOutputFormat::Interleaved,
                true, mNumThreads);
    setTimePoint("recver.expand.pprf_transpose");
...
void SilentOtExtReceiver::ferretMalCheck(std::shared_ptr<Channel> chl,
                                         PRNG &prng) {
  // MC_BEGIN(task<>, this, &chl, &prng, xx = block{}, sum0 = block{},
  //          sum1 = block{}, mySum = block{}, deltaShare = block{}, i = u64{},
  //          sender = NoisyVoleSender{}, theirHash = std::array<u8, 32>{},
  ...
  // MC_AWAIT(sender.send(mMalCheckX, {&deltaShare, 1}, prng, mMalCheckOts,
  // chl));
  ...
  // MC_AWAIT(chl.recv(theirHash));
  {
    ...
    auto status = fut.get();
    if (!status.IsOK()) {
      LOG(ERROR) << "Recv peer hash failed.";
      throw std::runtime_error("Recv peer hash failed.");
    }
  }
...
void SilentOtExtReceiver::hash(BitVector &choices, span<block> messages,
                               ChoiceBitPacking type) {
  ...
  if (type == ChoiceBitPacking::True) {
    ...
    for (u64 i = 0; i < n8; i += 8) {
      ...
      choices.data()[i / 8] = b0 ^ (b1 << 1) ^ (b2 << 2) ^ (b3 << 3) ^
                              (b4 << 4) ^ (b5 << 5) ^ (b6 << 6) ^ (b7 << 7);
...
void SilentOtExtReceiver::compress(ChoiceBitPacking packing) // )
{
  ...
  if (packing == ChoiceBitPacking::True) {
    ...
    case MultType::slv11:
      ...
      break;
    default:
      throw std::runtime_error("Only support slv5 and slv11.");
      break;
    }
