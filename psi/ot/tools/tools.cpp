...
void transpose(const MatrixView<u8> &in, const MatrixView<u8> &out) {
  ...
  for (int h = 0; h < subBlockHight; ++h) {
    ...
    for (int w = 0; w < subBlockWidth; ++w) {
      ...
      for (int j = 0; j < 8; j++) {
        // use the special movemask_epi8 to perform the final step of that
...
inline void sse_transposeSubSquarex(array<array<block, 8>, 128> &out,
  ...
  static_assert(sizeof(array<array<u16, 64>, 128>) ==
...
static OC_FORCEINLINE
  ...
  __m256i diff =
      _mm256_xor_si256(x, _mm256_slli_epi64(y, (u64)1 << blockSizeShift));
...
template <size_t blockSizeShift, size_t blockRowsShift, size_t j = 0>
static OC_FORCEINLINE typename std::enable_if<(j < (1 << blockSizeShift)) &&
...
template <size_t blockSizeShift, size_t matSizeShift, size_t blockRowsShift,
          size_t matRowsShift>
static OC_FORCEINLINE
    typename std::enable_if<blockSizeShift == matSizeShift>::type
...
void avx_transpose128(block *inOut) { avx_transpose((__m256i *)inOut); }
