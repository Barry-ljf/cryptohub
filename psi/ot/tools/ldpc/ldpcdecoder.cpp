...
std::vector<u8> LdpcDecoder::logbpDecode2(span<u8> codeword, u64 maxIter) {
  ...
  std::array<double, 3> wVal{
      {std::log(mP / (1 - mP)), std::log((1 - mP) / mP)}};
...
std::vector<u8> LdpcDecoder::altDecode(span<u8> codeword, bool minSum,
                                       u64 maxIter) {
  auto _N = mH.cols();
  std::array<double, 3> wVal{
      {std::log(mP / (1 - mP)), std::log((1 - mP) / mP)}};
...
std::vector<u8> LdpcDecoder::altDecode(span<double> w, bool min_sum,
                                       u64 maxIter) {
