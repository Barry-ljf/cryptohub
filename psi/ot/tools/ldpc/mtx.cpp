...
PointList::PointList(u64 r, u64 c, span<const Point> pp) : mRows(r), mCols(c) {
  for (auto p : pp)
    push_back(p);
}
void PointList::push_back(u64 x, u64 y) { push_back({x, y}); }
...
#define OC_INSERT_DEBUG
  ...
  for (u64 i = 0; i < u64(points.size()); ++i) {
    ...
    if (!s.second) {
      std::cout << "dup " << points[i].mRow << " " << points[i].mCol
                << std::endl;
      abort();
    }
    ...
    if (points[i].mCol >= cols) {
      std::cout << "col out of bounds " << points[i].mCol << " " << cols
                << std::endl;
      abort();
    }
#endif
...
bool SparseMtx::isSet(u64 row, u64 col) {
  ...
  auto iter = std::lower_bound(mCols[col].begin(), mCols[col].end(), row);
...
SparseMtx SparseMtx::subMatrix(u64 row, u64 col, u64 rowCount,
                               u64 colCount) const {
  ...
  for (u64 i = 0; i < rowIters.size(); ++i) {
    ...
    // std::transform(rowIters[i][0], rowIters[i][1], iter, [&](const auto& src)
    // {return src - col; });
  ...
  for (u64 i = 0; i < colIters.size(); ++i) {
    ...
    // std::transform(colIters[i][0], colIters[i][1], iter, [&](const auto& src)
    // {return src - row; });
...
SparseMtx SparseMtx::add(const SparseMtx &p) const {
  ...
  SparseMtx r;
  r.mDataCol.reserve(p.mDataCol.size() + mDataCol.size());
...
bool SparseMtx::operator==(const SparseMtx &X) const {
  auto eq = rows() == X.rows() && cols() == X.cols() &&
            mDataCol.size() == X.mDataCol.size() && mDataCol == X.mDataCol;
...
bool SparseMtx::operator!=(const SparseMtx &X) const { return !(*this == X); }
...
VecSortSet::iterator VecSortSet::begin() { return mData.begin(); }

VecSortSet::iterator VecSortSet::end() { return mData.end(); }
...
VecSortSet::constIerator VecSortSet::begin() const { return mData.begin(); }

VecSortSet::constIerator VecSortSet::end() const { return mData.end(); }
...
void VecSortSet::clear() { mData.clear(); }
...
void tests::Mtx_make_test() {
  ...
  sparse1.reserve(n1, n2);
  sparse1.resize(0, n2);
  sparse2.reserve(n1, n2);
  sparse2.resize(n1, 0);
  std::vector<Point> points;
...
#ifndef NDEBUG
  u64 cc = M00.mDataCol.size() + M01.mDataCol.size() + M10.mDataCol.size() +
           M11.mDataCol.size(),
      rr = M00.mDataRow.size() + M01.mDataRow.size() + M10.mDataRow.size() +
           M11.mDataRow.size();
#endif
