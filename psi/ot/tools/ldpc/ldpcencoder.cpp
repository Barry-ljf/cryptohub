...
namespace details {
constexpr std::array<std::array<u8, 4>, 16>
    SilverRightEncoder::diagMtx_g16_w5_seed1_t36;
constexpr std::array<std::array<u8, 10>, 32>
    SilverRightEncoder::diagMtx_g32_w11_seed2_t36;
...
void DiagInverter::mult(const SparseMtx &y, SparseMtx &x) {
  ...
  std::vector<u64> &xCol = x.mDataCol;
  xCol.reserve(y.mDataCol.size());
  std::vector<u64> colSizes(xNumCols), rowSizes(xNumRows);
  ...
  for (u64 i = 0; i < colSizes.size(); ++i) {
    ...
    x.mCols[i] =
        end - iter ? SparseMtx::Col(span<u64>(iter, end)) : SparseMtx::Col{};
    iter = end;
  }
  ...
  for (u64 i = 0; i < rowSizes.size(); ++i) {
    ...
    x.mRows[i] =
        SparseMtx::Row(iter != end ? span<u64>(iter, end) : span<u64>{});
    iter = end;
  }
...
void SilverLeftEncoder::init(u64 rows, std::vector<double> rs, u64 rep) {
  ...
  for (u64 i = 0; i < mWeight; ++i) {
    ...
    while (
        //(rep && reps[mYs[i] % rep]) ||
        (rep && mYs[i] % rep) || s.insert(mYs[i]).second == false) {
      mYs[i] = (mYs[i] + 1) % rows;

      if (++trials > 1000) {
        std::cout << "these ratios resulted in too many collisions. " LOCATION
                  << std::endl;
        throw std::runtime_error(
            "these ratios resulted in too many collisions. " LOCATION);
      }
    }
...
void SilverLeftEncoder::init(u64 rows, SilverCode code, u64 rep) {
  ...
  case 11:
    init(rows,
         {{0, 0.00278835, 0.0883852, 0.238023, 0.240532, 0.274624, 0.390639,
           0.531551, 0.637619, 0.945265, 0.965874}},
         rep);
    // 0 0.00278835 0.0883852 0.238023 0.240532 0.274624 0.390639 0.531551
    // 0.637619 0.945265 0.965874
    break;
  default:
...
void tests::LdpcEncoder_encode_test() {
  ...
  for (u64 i = 0; i < 40; ++i) {
    ...
    // std::cout << " +====================" << std::endl;
    while (b) {
      H = sampleTriangularBand(rows, cols, colWeight, gap, dWeight, false,
                               prng);
...
void tests::LdpcEncoder_encode_g0_test() {
  ...
  for (u64 i = 0; i < 40; ++i) {
    ...
    while (b) {
      ...
      H = sampleTriangularBand(rows, cols, colWeight, 8, colWeight, colWeight,
                               0, 0, {5, 31}, true, true, true, prng, prng);
      // H = sampleTriangular(rows, cols, colWeight, gap, prng);
      b = !E.init(H, 0);
    }
