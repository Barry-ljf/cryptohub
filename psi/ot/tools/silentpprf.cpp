...
void SilentMultiPprfSender::setBase(
    span<const std::array<block, 2>> baseMessages) {
...
void SilentMultiPprfReceiver::setBase(span<const block> baseMessages) {
  ...
  memcpy(mBaseOTs.data(), baseMessages.data(),
         baseMessages.size() * sizeof(block));
}
...
// @mal         - ...
void copyOut(span<AlignedArray<block, 8>> lvl, MatrixView<block> output,
             u64 totalTrees, u64 tIdx, PprfOutputFormat oFormat,
             std::function<void(u64 treeIdx, span<AlignedArray<block, 8>> lvl)>
                 &callback) {
...
u64 interleavedPoint(u64 point, u64 treeIdx, u64 totalTrees, u64 domain,
...
BitVector SilentMultiPprfReceiver::sampleChoiceBits(u64 modulus,
                                                    PprfOutputFormat format,
                                                    PRNG &prng) {
...
void SilentMultiPprfReceiver::setChoiceBits(PprfOutputFormat format,
                                            BitVector choices) {
...
// 2*j+1  on level i+1.
span<AlignedArray<block, 8>> SilentMultiPprfSender::Expander::getLevel(u64 i,
                                                                       u64 g) {
...
SilentMultiPprfSender::Expander::Expander(SilentMultiPprfSender &p, block seed,
                                          u64 treeIdx_, PprfOutputFormat of,
                                          MatrixView<block> o,
                                          bool activeChildXorDelta,
                                          std::shared_ptr<Channel> s)
    : pprf(p), chl(std::move(s)), mActiveChildXorDelta(activeChildXorDelta) {
  treeIdx = treeIdx_;
  assert((treeIdx & 7) == 0);
  output = o;
  oFormat = of;
...
void SilentMultiPprfSender::Expander::run() {
#ifdef DEBUG_PRINT_PPRF
  chl.asyncSendCopy(mValue);
#endif

  // pprf.setTimePoint("SilentMultiPprfSender.begin " +
  // std::to_string(treeIdx));
  {
  ...
  // pprf.setTimePoint("SilentMultiPprfSender.alloc " +
  // std::to_string(treeIdx));
  ...
  // the sets of trees are processed with the other threads.
  {
...
#ifdef DEBUG_PRINT_PPRF
        if (mPrint) {
          std::cout << "c[" << treeIdx + j << "][" << d << "][0] "
                    << sums[0][d][j] << " " << mBaseOTs[treeIdx + j][d][0]
                    << std::endl;
          ;
          std::cout << "c[" << treeIdx + j << "][" << d << "][1] "
                    << sums[1][d][j] << " " << mBaseOTs[treeIdx + j][d][1]
                    << std::endl;
          ;
        }
#endif
    ...
    // pprf.setTimePoint("SilentMultiPprfSender.expand " +
    // std::to_string(treeIdx));
    ...
    if (mActiveChildXorDelta) {
      ...
      // pprf.setTimePoint("SilentMultiPprfSender.last " +
      // std::to_string(treeIdx));
    ...
    auto status = chl->asyncSend(std::move(sums[0]));
    if (!status.IsOK()) {
      LOG(ERROR) << "Send sums[0] failed.";
      throw std::runtime_error("Send sums[0] failed.");
    }

    status = chl->asyncSend(std::move(sums[1]));
    if (!status.IsOK()) {
      LOG(ERROR) << "Send sums[1] failed.";
      throw std::runtime_error("Send sums[1] failed.");
    }

    if (mActiveChildXorDelta) {
      auto status = chl->asyncSend(std::move(lastOts));
      if (!status.IsOK()) {
        LOG(ERROR) << "Send lastOts failed.";
        throw std::runtime_error("Send lastOts failed.");
      }
    ...
    // pprf.setTimePoint("SilentMultiPprfSender.copyOut " +
    // std::to_string(treeIdx));
  }
  ...
  // pprf.setTimePoint("SilentMultiPprfSender.delete " +
  // std::to_string(treeIdx));
}
...
void SilentMultiPprfSender::expand(std::shared_ptr<Channel> chl,
                                   span<const block> value, PRNG &prng,
                                   MatrixView<block> output,
                                   PprfOutputFormat oFormat,
  ...
  // MC_BEGIN(task<>, this, numThreads, oFormat, output, &prng, &chl,
  //          activeChildXorDelta, i = u64{}, dd = u64{});
  u64 i = 0;
  u64 dd = 0;
  ...
  mExps.clear();
  mExps.reserve(divCeil(mPntCount, 8));

  for (i = 0; i < mPntCount; i += 8) {
    mExps.emplace_back(*this, prng.get(), i, oFormat, output,
                       activeChildXorDelta, chl->fork());
    // mExps.back().mFuture = macoro::make_eager(mExps.back().run());
    // MC_AWAIT(mExps.back().run());
    Expander *inst = &(mExps.back());
    auto run_fn = [inst]() { inst->run(); };
    mExps.back().mFuture = std::async(run_fn);
  }
...
// children of node j on level i are located at 2*j and
// 2*j+1  on level i+1.
span<AlignedArray<block, 8>>
SilentMultiPprfReceiver::Expander::getLevel(u64 i, u64 g, bool f) {
  // auto size = (1ull << i);
#ifdef DEBUG_PRINT_PPRF
  // auto offset = (size - 1);
...
SilentMultiPprfReceiver::Expander::Expander(SilentMultiPprfReceiver &p,
                                            std::shared_ptr<Channel> s,
                                            PprfOutputFormat of,
                                            MatrixView<block> o,
                                            bool activeChildXorDelta, u64 ti)
    : pprf(p), chl(std::move(s)), mActiveChildXorDelta(activeChildXorDelta),
      oFormat(of), output(o), treeIdx(ti)
//, threadIdx(tIdx)
{
  assert((treeIdx & 7) == 0);
  // A public PRF/PRG that we will use for deriving the GGM tree.
...
void SilentMultiPprfReceiver::Expander::run() {
  {
    tree = pprf.mTreeAlloc.get();
    assert(tree.size() >= 1ull << (dd));
    mLevels.resize(dd);
  ...
  // the sets of trees are processed with the other threads.
  {
    ...
    auto fut1 = chl->asyncRecv(theirSums[0]);
    fut1.get();
    auto fut2 = chl->asyncRecv(theirSums[1]);
    fut2.get();

    // MC_AWAIT(chl.recv(lastOts));
    if (mActiveChildXorDelta) {
      auto fut = chl->asyncRecv(lastOts);
      fut.get();
    }

    // pprf.setTimePoint("SilentMultiPprfReceiver.recv " +
    // std::to_string(treeIdx));
    ...
    // pprf.setTimePoint("SilentMultiPprfReceiver.alloc " +
    // std::to_string(treeIdx));
    ...
    for (u64 i = 0; i < 8; ++i) {
      ...
      if (neq(l1[notAi][i], l1f[notAi][i])) {
        std::cout << "l1[" << notAi << "][" << i << "] " << l1[notAi][i]
                  ...
                  << theirSums[notAi][0][i] << " vs " << l1f[notAi][i]
                  << std::endl;
      }
#endif
    }
    ...
    for (u64 d = 1; d < pprf.mDepth; ++d) {
      ...
      if (!mActiveChildXorDelta || d != pprf.mDepth - 1) {
        ...
        for (u64 i = 0; i < 8; ++i) {
          ...
          inactiveChild =
              correctSum ^ mySums[notAi][i] ^ pprf.mBaseOTs[i + treeIdx][d];

#ifdef DEBUG_PRINT_PPRF
          if (mPrint)
            std::cout << "up[" << i << "] = level1[" << inactiveChildIdx << "]["
                      << i << "] " << prev << " -> "
                      << level1[inactiveChildIdx][i] << " " << activeChildIdx
                      << " " << inactiveChildIdx << " ~~ "
                      << mBaseOTs[i + treeIdx][d] << " "
                      << theirSums[notAi][d][i] << " @ " << (i + treeIdx) << " "
                      << d << std::endl;
    ...
    // pprf.setTimePoint("SilentMultiPprfReceiver.expand " +
    // std::to_string(treeIdx));
    ...
    if (mActiveChildXorDelta) {
      ...
      for (u64 j = 0; j < 8; ++j) {
        ...
        if (mPrint)
          std::cout << "up[" << d << "] = level1["
                    << (inactiveChildIdx / mPntCount) << "]["
                    << (inactiveChildIdx % mPntCount) << " " << prev << " -> "
                    << level[inactiveChildIdx][j] << " ~~ "
                    << mBaseOTs[j + treeIdx][d] << " " << ot0 << " @ "
                    << (j + treeIdx) << " " << d << std::endl;
#endif
      }
      // pprf.setTimePoint("SilentMultiPprfReceiver.last " +
      // std::to_string(treeIdx));
    ...
    // pprf.setTimePoint("SilentMultiPprfReceiver.copy " +
    // std::to_string(treeIdx));
    ...
    // pprf.setTimePoint("SilentMultiPprfReceiver.delete " +
    // std::to_string(treeIdx));
  }
}

void SilentMultiPprfReceiver::expand(std::shared_ptr<Channel> chl, PRNG &prng,
                                     MatrixView<block> output,
                                     PprfOutputFormat oFormat,
                                     bool activeChildXorDelta, u64 numThreads) {
  ...
  // lout << " d " << mDomain << " p " << mPntCount << " do " << mDepth <<
  // std::endl;
  ...
  // MC_BEGIN(task<>, this, numThreads, oFormat, output, &chl,
  // activeChildXorDelta,
  //          i = u64{}, dd = u64{});
  u64 i = 0;
  u64 dd = 0;
  ...
  mExps.reserve(divCeil(mPntCount, 8));
  for (i = 0; i < mPntCount; i += 8) {
    mExps.emplace_back(*this, chl->fork(), oFormat, output, activeChildXorDelta,
                       i);
    Expander *inst = &(mExps.back());
    auto run_fn = [inst] { inst->run(); };
    mExps.back().mFuture = std::async(run_fn);
