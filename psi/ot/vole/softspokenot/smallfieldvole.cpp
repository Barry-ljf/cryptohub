...
// Output: v (or w) in inOut[1 ...], and u in inOut[0]. If blocks > 2**depth,
// this pattern repeats once every 2**depth blocks.
...
// Base case
template <>
OC_FORCEINLINE void xorReduce<0, 0>(block *__restrict inOut, u64 maxDepth) {}

// outU is unused when this is called by the receiver, and a few operations
// might be saved by not computing it. However, it's just a few XORs, and the
// compiler will likely optimize it out anyway.
static OC_FORCEINLINE void
xorReducePath(u64 fieldBits, u64 fieldSize, u64 superBlk,
              block (*__restrict path)[superBlkSize], block *__restrict outU,
              block *__restrict outVW, bool isReceiver,
              bool correctionPresent = false) {
  // Reduce up the combining tree, continuing for as many nodes just got
  // completed. However, the root is skipped, in case it might have a different
  // size.
  u64 treeDepth = 0;
  for (u64 blkInTree = superBlk; treeDepth < (fieldBits - 1) / superBlkShift &&
                                 blkInTree % superBlkSize == 0;
       ++treeDepth) {
...
OC_FORCEINLINE void generateSenderImpl(const SmallFieldVoleSender &This,
                                       u64 blockIdx, const AES &aes,
                                       block *__restrict outU,
                                       block *__restrict outV) {
  ...
  if (fieldBits <= superBlkShift) // >= 1 VOLEs per superblock.
  {
    ...
    for (u64 nVole = 0; nVole < This.mNumVoles; nVole += volePerSuperBlk) {
      // TODO: Try combining the block index into the AES key. It gives the same
      // PRG, but might be faster.
  ...
  {
    for (u64 nVole = 0; nVole < This.mNumVoles;
         ++nVole, outV += fieldBits, ++outU) {
...
template <u64 fieldBitsConst>
OC_FORCEINLINE void generateReceiverImpl(const SmallFieldVoleReceiver &This,
                                         u64 blockIdx, const AES &aes,
                                         block *__restrict outW,
                                         const block *__restrict correction) {
  // Allow the compiler to hardcode mFieldBits based on the template parameter.
  const u64 fieldBits = fieldBitsConst > 0 ? fieldBitsConst : This.mFieldBits;
  constexpr u64 fieldBitsMax =
      fieldBitsConst > 0 ? fieldBitsConst : SmallFieldVoleSender::fieldBitsMax;
  ...
  if (fieldBits <= superBlkShift) {
    // >= 1 VOLEs per superblock. This case can only occur for small fieldSize,
    // small enough that this cannot be the variable size implementation
    // (fieldBitsConst != 0).
    if (fieldBitsConst == 0)
      UNREACHABLE();
    ...
    // per super block, and how many AES calls to use. Try to get as close to 8
    // AES invocations per superblock as possible.
    ...
    for (u64 nVole = 0; nVole < This.mNumVoles; nVole += volePerSuperBlk,
      ...
      block input[aesPerSuperBlk], hashes[aesPerSuperBlk],
          xorHashes[fieldsPerSuperBlk];
      for (u64 i = 0; i < aesPerSuperBlk; ++i, ++seeds)
        input[i] = blockIdxBlock ^ *seeds;
      aes.hashBlocks<aesPerSuperBlk>(input, hashes);

      // Intersperse the hashes with zeros, because the zeroth seed for each
      // VOLE is unknown.
    ...
    // > 1 super block per VOLE. Do blocks of 8, or 7 at the start because the
    // zeroth seed in a VOLE is unknown.
    for (u64 nVole = 0; nVole < This.mNumVoles;
         ++nVole, outW += fieldBits, deltaPtr += fieldBits) {
      block path[divCeil(fieldBitsMax, superBlkShift)][superBlkSize];
      if (correctionPresent)
        for (u64 i = 0; i < fieldBits; ++i)
      ...
      // The zeroth seed is unknown, so set the corresponding path element to
      // zero.
...
// mSeeds must be the OT messages from mNumVoles instances of 2**mFieldBits - 1
// of 2**mFieldBits OT, with each OT occupying a contiguous memory range.
...
u64 computeNumVolesPadded(u64 fieldBits, u64 numVoles) {
  ...
  if (fieldBits <= superBlkShift) // >= 1 VOLEs per superblock.
    volesPadded =
        roundUpTo(numVoles, divNearest(superBlkSize, (1 << fieldBits) - 1));
  else // > 1 super block per VOLE.
...
void SmallFieldVoleReceiver::init(u64 fieldBits_, u64 numVoles_,
                                  bool malicious) {
  SmallFieldVoleBase::init(fieldBits_, numVoles_, malicious);
  mGenerateFn = (selectGenerateImpl(mFieldBits));
  if (!mPprf)
...
// The choice bits (as expected by SilentMultiPprfReceiver) store the locations
// of the complements of the active paths (because they tell which messages were
// transferred, not which ones weren't), in big endian. We want delta, which is
// the locations of the active paths, in little endian.
static BitVector choicesToDelta(const BitVector &choices, u64 fieldBits,
                                u64 numVoles) {
...
void SmallFieldVoleReceiver::setBaseOts(span<const block> baseMessages,
...
void SmallFieldVoleSender::expand(std::shared_ptr<Channel> chl,
  ...
  std::vector<std::array<block, 2>> corrections = {};
  std::vector<std::array<block, 2>> hashes = {};
  MatrixView<block> seedView;
  ...
  mPprf->expand(chl, span<const block>(), prng, seedView,
                PprfOutputFormat::BlockTransposed, false, 1);
  ...
  if (mMalicious) {
    ...
    auto status = chl->asyncSend(std::move(corrections));
    if (!status.IsOK()) {
      LOG(ERROR) << "Send corrections failed.";
      throw std::runtime_error("Send corrections failed.");
    }

    status = chl->asyncSend(std::move(hashes));
    if (!status.IsOK()) {
      LOG(ERROR) << "Send hashes failed.";
      throw std::runtime_error("Send hashes failed.");
    }
  }
}

void SmallFieldVoleReceiver::expand(std::shared_ptr<Channel> chl,
                                    PRNG &prng, u64 numThreads) {
  // MC_BEGIN(task<>, this, &chl, &prng, numThreads, seedsFull =
  // Matrix<block>{},
  ...
  Matrix<block> seedsFull = {};
  std::vector<std::array<block, 2>> totals = {};
  std::vector<std::array<block, 2>> entryHashes = {};
  std::vector<std::array<block, 2>> corrections = {};
  std::vector<std::array<block, 2>> hashes = {};
  block *seedMatrix = nullptr;
  ...
  mPprf->expand(chl, prng, seedsFull, PprfOutputFormat::BlockTransposed, false,
                1);
  ...
  if (mMalicious) {
    ...
    auto fut = chl->asyncRecv(corrections);
    auto status = fut.get();
    if (!status.IsOK()) {
      LOG(ERROR) << "Recv corrections failed.";
      throw std::runtime_error("Recv corrections failed.");
    }

    fut = chl->asyncRecv(hashes);
    status = fut.get();
    if (!status.IsOK()) {
      LOG(ERROR) << "Recv hashes failed.";
      throw std::runtime_error("Recv hashes failed.");
    }

    {
      ...
      for (u64 row = 0; row < mNumVoles; ++row) {
        ...
        for (u64 col = 0; col < fieldSize(); ++col) {
          ...
          for (int i = 0; i < 2; ++i)
            entryHashes[row * fieldSize() + col][i] ^=
                isUnknownSeed & corrections[row][i];
        }
      ...
      // TODO: Should delay abort until the VOLE consistency check, to stop the
      // two events from being distinguished.
      if (!eq)
  ...
  }
  {
    ...
    // Reorder mSeeds to handle the (Delta ^) part, moving the unknown mSeeds to
    // column 0. This effectively makes it compute w = sum_x (Delta ^ x) * r_x
    // instead of v = sum_x x * r_x.
    // TODO: Might be best to do this as part of going down the pprf tree.
    // There, it would cost O(mNumVoles * fieldSize) time, rather than
    // O(mNumVoles * fieldSize * mFieldBits).
    for (u64 i = 0; i < mFieldBits; ++i) {
      u64 dMask = (u64)1 << i;
      block *matrix = seedsFull.data();
      // Bit hack to iterate over all values that are have a 0 in position i
      // (from Hacker's Delight).
      for (u64 col = 0; col < fieldSize(); col = (col + dMask + 1) & ~dMask)
        for (u64 row = 0; row < mNumVoles; ++row)
          cswapBytes(matrix[row * fieldSize() + col],
                     matrix[row * fieldSize() + col + dMask],
                     mDeltaUnpacked[row * mFieldBits + i]);
    }

    // Remove the unknown mSeeds so that generate doesn't have to skip over
    // them.
...
// TODO: Malicious version. Should use an actual hash function for bottom layer
// of tree.

SmallFieldVoleSender::GenerateFn
SmallFieldVoleSender::selectGenerateImpl(u64 fieldBits) {
  static const GenerateFn table[] = {
      generateSenderImpl<1>, generateSenderImpl<2>, generateSenderImpl<3>,
      generateSenderImpl<4>, generateSenderImpl<5>, generateSenderImpl<6>,
      generateSenderImpl<7>, generateSenderImpl<8>, generateSenderImpl<9>,
...
SmallFieldVoleReceiver::selectGenerateImpl(u64 fieldBits) {
  static const GenerateFn table[] = {
      generateReceiverImpl<1>, generateReceiverImpl<2>, generateReceiverImpl<3>,
      generateReceiverImpl<4>, generateReceiverImpl<5>, generateReceiverImpl<6>,
      generateReceiverImpl<7>, generateReceiverImpl<8>, generateReceiverImpl<9>,
...
void tests::xorReduction() {
  ...
  const std::array<block, 8> testSuperBlock = {
      block::allSame(0x20), block::allSame(0x03), block::allSame(0xf9),
      block::allSame(0x2d), block::allSame(0x3a), block::allSame(0xb9),
      block::allSame(0xa3), block::allSame(0xbb)};

  std::array<block, 8> depth1 = testSuperBlock;
  const std::array<block, 8> desiredDepth1 = {
      block::allSame(0x23), block::allSame(0x03), block::allSame(0xd4),
      block::allSame(0x2d), block::allSame(0x83), block::allSame(0xb9),
      block::allSame(0x18), block::allSame(0xbb)};
  ...
  const std::array<block, 8> desiredDepth2 = {
      block::allSame(0xf7), block::allSame(0x2e), block::allSame(0xd4),
      block::allSame(0x2d), block::allSame(0x9b), block::allSame(0x02),
      block::allSame(0x18), block::allSame(0xbb)};
  ...
  const std::array<block, 8> desiredDepth3 = {
      block::allSame(0x6c), block::allSame(0x2c), block::allSame(0xcc),
      block::allSame(0x9b), block::allSame(0x9b), block::allSame(0x02),
      block::allSame(0x18), block::allSame(0xbb)};
