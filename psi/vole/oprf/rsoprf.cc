...
void RsOprfSender::send(u64 n, PRNG &prng, const std::shared_ptr<Channel> &chl,
  ...
  //     recvIdx = u64{ 0 },
  //     fork = std::shared_ptr<Channel>&{}
  // );

  block ws;
  std::array<u8, 32> hBuff;
  oc::RandomOracle ro(32);
  std::unique_ptr<block[]> pPtr;
  span<block> pp;
  span<block> subPp;
  span<block> remB;
  span<block> subB;
  // macoro::eager_task<void> fu;
  u64 recvIdx;
  ...
  mPaxos.init(n, mBinSize, 3, mSsp, crypto::okvs::PaxosParam::GF128,
              oc::ZeroBlock);

  mD = prng.get();

  if (mMalicious) {
    mVoleSender.mMalType = crypto::SilentSecType::Malicious;
    ro.Update(ws);
    ro.Final(hBuff);
    chl->send(std::move(hBuff));
    // MC_AWAIT(chl->send(std::move(hBuff)));
  }

  if (mTimer) mVoleSender.setTimer(*mTimer);
  ...
  chl->recv(mPaxos.mSeed);

  std::clock_t start = clock();
  genVole(prng, fork, reducedRounds);
  std::clock_t end = clock();
  //std::cout << "RsOprfSender genVole" << "cost " << (double)(end - start) / CLOCKS_PER_SEC << "sec." << std::endl;
...
void RsOprfSender::eval(span<const block> val, span<block> output,
                        u64 numThreads) {
...
void RsOprfSender::genVole(PRNG &prng, const std::shared_ptr<Channel> &chl,
                           bool reduceRounds) {
  if (reduceRounds)
    mVoleSender.configure(mPaxos.size(), crypto::SilentBaseType::Base);
...
void RsOprfReceiver::receive(span<const block> values, span<block> outputs,
                             PRNG &prng, const std::shared_ptr<Channel> &chl,
                             u64 numThreads, bool reducedRounds) {
  ...
  block ws;
  std::array<u8, 32> Hws;
  crypto::okvs::Baxos paxos;
  std::unique_ptr<block[]> hPtr;
  span<block> h;
  UninitVec p;
  span<block> subP;
  span<block> subC;
  ...
  if (values.size() != outputs.size()) throw RTE_LOC;

  hashingSeed = prng.get(), wr = prng.get();
  paxos.mDebug = mDebug;
  paxos.init(values.size(), mBinSize, 3, mSsp, crypto::okvs::PaxosParam::GF128,
             hashingSeed);

  // MC_AWAIT(chl->send(std::move(hashingSeed)));
  chl->send(std::move(hashingSeed));

  if (mMalicious) {
    mVoleRecver.mMalType = crypto::SilentSecType::Malicious;
    // MC_AWAIT(chl->recv(Hws));
    chl->recv(Hws);
  }

  if (mTimer) mVoleRecver.setTimer(*mTimer);
  ...
  // macoro::make_eager();
  std::clock_t start = clock();
  genVole(paxos.size(), prng, fork, reducedRounds);
  std::clock_t end = clock();
  //std::cout << "RsOprfReceiver genVole" << "cost " << (double)(end - start) / CLOCKS_PER_SEC << "sec." << std::endl;
  ...
  if (0) {
    {
      ...
      for (u64 i = main; i < p.size(); ++i) p[i] = p[i] ^ c[i];
    }
    ...
    while (c.size()) {
...

      {
        ...
        for (u64 i = main; i < p.size(); ++i, ++pp, ++cc) *pp = *pp ^ *cc;
      }
...
void RsOprfReceiver::genVole(u64 n, PRNG &prng,
                             const std::shared_ptr<Channel> &chl,
                             bool reducedRounds) {
  if (reducedRounds) mVoleRecver.configure(n, crypto::SilentBaseType::Base);
